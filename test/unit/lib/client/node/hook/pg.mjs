...
const proceed = async () => {
  ...
  client.connect();
  // promise //
  Assert.deepEqual((await client.query("SELECT $1::integer * $2::integer AS solution;", [2, 3])).rows, [ { solution: 6 } ]);
  try {
...
if (Reflect.getOwnPropertyDescriptor(process.env, 'TRAVIS')) {
  ...
  {
    const { signal, status } = ChildProcess.spawnSync('rm', ['-rf', PATH], { stdio: 'inherit' });
    Assert.equal(signal, null);
    Assert.equal(status, 0);
  }
  {
    const { signal, status } = ChildProcess.spawnSync('initdb', ['--no-locale', '--encoding', 'UTF-8', '--pgdata', PATH, '--username', USER], { stdio: 'inherit' });
    Assert.equal(signal, null);
    Assert.equal(status, 0);
  }
  const child = ChildProcess.spawn('postgres', ['-D', PATH, '-p', String(PORT)], { stdio: 'inherit' });
  process.on('exit', () => {
    child.kill('SIGINT');
  });
  const probe = () => {
