...
import {getDisabledRecorder, start, startSync} from '../../../../../lib/client/es2015/recorder.js';
...
((async () => {
  ...
  // Disabled - Sync //
  Assert.equal(
    getDisabledRecorder().isEnabled(),
    false
  );
  Assert.equal(
    getDisabledRecorder().pauseSync(),
    undefined
  );
  Assert.equal(
    getDisabledRecorder().playSync(),
    undefined
  );
  Assert.equal(
    getDisabledRecorder().stopSync(),
    undefined
  );
