...
import {makeAppmap, makeAppmapSync} from '../../../../../lib/client/es2015/appmap.js';
import {getDisabledRecorder} from '../../../../../lib/client/es2015/recorder.js';


((async () => {

  const options = {process: "process", navigator:"navigator", configuration:"configuration"}
...

  {
    ...
    const respond = (json) => {
      ...
      if (json.action === "initialize") {
        counter += 1;
        return {enabled: true, session: "session", namespace: `HIDDEN${String(counter)}`};
      }
...

    {
      ...
      Assert.throws(
        () => makeAppmapSync({requestSync: () => { Assert.fail(); }, request: () => { Assert.fail(); }}, hook, run, options),
...

    {
      ...
      Assert.equal(await new Promise((resolve, reject) => { hook.instrument("source", "path", "content", {resolve, reject}); }), null);
      Assert.equal(getRuntime().record("origin", "event"), undefined);
...

      {
        const appmap = await makeAppmap({
          ...
          request: (...args) => {
            ...
            return args[1].resolve({enabled:false, session:null, namespace:null});
          }
        }, hook, run, options);
