...
import {toEither, isLeft, isRight, fromLeft, fromRight, Left, Right} from '../../../../lib/server/either.mjs';
...
Assert.equal(
  new Right("foo").mapLeft(() => { Assert.fail() }).fromRight(),
  "foo"
);

Assert.equal(
  new Left("foo").mapLeft((...args) => {
    Assert.deepEqual(args, ["foo"]);
    return "bar";
  }).fromLeft(),
  "bar"
...
Assert.equal(
  isRight(new Right('foo')),
  true
);

Assert.equal(
  fromLeft(new Left('foo')),
  "foo"
);

Assert.equal(
  fromRight(new Right('foo')),
  "foo"
);
