...

{
  ...
  Assert.deepEqual(childeren, normalizeChild({
    type: "spawn",
    exec: "node",
    argv: ["/main.js"]
  }));
...
Assert.equal(
  ...
  FileSystem.readdirSync(".").filter((name) => (
    !name.startsWith(".") &&
    !FileSystem.lstatSync(name).isDirectory())
  ).length
);
...
Assert.equal(
  spawnNormalizedChild(child, mock).fromRight(),
  "foo"
);
...
mock.protocol = "messaging";
mock.port = 8080;
setSpawnForTesting(() => { throw new Error("foo") });
Assert.match(
  spawnNormalizedChild(child, mock).fromLeft(),
  /foo$/
);
