...

{
  ...
  Assert.equal(file.parse().fromRight().type, 'Program');
}

Assert.equal(
  new File(2015, 'script', 'tmp/test/script-es2015.js').parse().fromRight()
    .type,
  'Program',
);

Assert.equal(
  new File(2015, 'module', 'tmp/test/module-es2015.js').parse().fromRight()
    .type,
  'Program',
);
