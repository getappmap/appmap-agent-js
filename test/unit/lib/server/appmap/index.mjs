...
((async () => {
  ...
  Assert.equal(
    (await appmap.initializeAsync("$")).fromRight(),
    "$"
  );

  const key = (await appmap.startAsync({
    data: {
      base: {
        path: "/base"
      },
  ...
  Assert.ok(
    (await appmap.instrumentAsync({source:'script', path:'/base/qux/invalid-main.js', content:'@invalid'})).isLeft(),
  );

  Assert.ok(
    (await appmap.instrumentAsync({source:'script', path:'base/qux/relative-main.js', content:'123;'})).isLeft(),
  );

  Assert.equal(
    typeof (await appmap.instrumentAsync({source:'script', path:'/base/qux/main.js', content:'123;'})).fromRight(),
    "string"
  );
  ...
  const json = JSON.parse(FileSystem.readFileSync("tmp/test/bar.appmap.json", "utf8"));
...

{
  ...
  Assert.equal(
    appmap.stop(key1).fromRight(),
    null
  );
  const key2 = appmap.start({data:{}, path:process.cwd()}).fromRight();
  appmap.stopAsync(key2).then((either) => Assert.equal(
    either.fromRight(),
    null
  ));
  // key3
