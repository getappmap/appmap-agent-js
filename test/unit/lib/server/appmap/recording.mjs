...

{
  ...
  const recording = new Recording(getInitialConfiguration().extendWithData({
  ...
  const appmap = JSON.parse(FileSystem.readFileSync("tmp/test/bar.appmap.json", "utf8"));
  Assert.deepEqual(
    appmap.classMap,
    [
      {
...

{
  ...
  const appmap = JSON.parse(FileSystem.readFileSync("tmp/test/foo.appmap.json", "utf8"));
  Assert.deepEqual(
    appmap.classMap,
    [
      {
...
Assert.match(
  new Recording(getInitialConfiguration().extendWithData({
    "output": {
      "directory-path": "missing",
      "file-name": "foo"
    }
  }, process.cwd()).fromRight()).terminate(identity).fromLeft(),
...
new Recording(getInitialConfiguration().extendWithData({
  ...
  }, process.cwd()).fromRight()).terminateAsync(identity).then((either) => {
