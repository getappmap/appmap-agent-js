...
export default (env) => {
  ...
  return {
    ...
    instrumentScript: (content, path) => {
      ...
      return instrument(new File(appmap.getLanguageVersion(), "script", path, content), appmap.getNamespace(), addEntity);
    },
    instrumentModule: (content, path, pending) => {
      if (appmap === null) {
        ...
        pending.resolve(new File(appmap.getLanguageVersion(), "script", path, content), appmap.getNamespace(), addEntity);
      }
    },
    emit: (event) => {
      if (appmap === null) {
