...
import { nomralizeChild } from "./child.mjs";
...
const normalizeOutput = (output) => {
  ...
  if (Reflect.getOwnPropertyDescriptor(output, 'directory-path') === undefined) {
    return output;
  }
  return {
    ...output,
...
const normalizePackages = (specifiers) => specifiers.flatMap(normalizeSpecifier);
...
const normalizeEnabled = (specifiers) => {
  ...
  return specifiers.flatMap(normalizeSpecifier(specifier));
};
...
export const makeInitialFieldObject = (name) => {
  ...
  Reflect.ownKeys(infos).forEach((name) => {
    data[name] = infos[name].initial;
  });
  return data;
};

export const extendField = (data, key, value) => {
  assert(key in infos, `invalid field key %o`, name);
