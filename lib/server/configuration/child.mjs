...
export const spawnNormalizedChild = (child, configuration) => {
  ...
  if (configuration.getProtocol() === 'inline') {
    either = configuration.extendWithData(child.configuration, child.base).mapRight(({data}) => ({
      ...env,
      ...child.env,
  ...
  // NB: no idea why we need to ignore the finally below, probably a c8 bug
  /* c8 ignore start */ } finally { /* c8 ignore stop */
    process.chdir(save);
  }
};
