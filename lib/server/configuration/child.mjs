...
export const normalizeChild = (child) => {
  ...
  if (child.type === 'spawn') {
      ...
      return [
        {
          type: 'normalized',
          base,
          command: [...child.launcher, child.exec, ...child.argv],
    ...
    if (child.type === 'fork') {
      ...
      return (child.globbing
        ...
        argv: [
          ...
          '--require',
          getRecorderPath(child['node-version'], 'normal'),
          main,
          ...child.argv,
        ],
        base,
    ...
    /* c8 ignore start */
    assert(false, 'invalid child recorder %o', child);
    /* c8 ignore stop */
  }
  /* c8 ignore start */
...
export const spawnNormalizedChild = (child, configuration) => {
  ...
  try {
    return either.mapRight((env) =>
      spawn(child.argv[0], child.argv.slice(1, child.argv.length), {
        ...child.options,
        env,
      }),
    );
