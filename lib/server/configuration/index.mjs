...
const getInstrumentationDefault = { enabled: false, shallow: false, exclude: [] }
...
class Configuration {
  ...
  extendWithData (data2, path, done = []) {
    ...
    return validateConfiguration(data2).bind((data2) => changeWorkingDirectory(path, () => {
  ...
  isEnabled() {
    ...
    return new Right(lookupGroupArray(this.data.enabled, this.data.main.path, isEnabledDefault).enabled);
  }
  isClassMapPruned() {
  ...
  spawnChild (child) {
    return spawnNormalizedChild(child, (data, path) => this.extendWithData(data, path));
  }
}
