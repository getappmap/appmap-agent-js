...
import { lookupGroupArray } from './group.mjs';
...
class Configuration {
  ...
  extendWithData(data2, base, done = []) {
    return validateConfigurationData(data2).then(() =>
      changeWorkingDirectory(base, () => {
        ...
        return either.fmap(({ data: data1 }) => {
          data1 = { __proto__: null, ...data1 };
          Reflect.ownKeys(data2).forEach((name) => {
            extendField(data1, name, data2[name]);
          });
  ...
  isEnabled() {
    assert(this.data.main.path !== null, 'missing main path for enable query');
    return lookupGroupArray(this.data.enabled, this.data.main.path, {
      enabled: false,
    }).enabled;
  }
  isClassMapPruned() {
  ...
  getInstrumentation(path) {
    const { enabled, shallow, exclude } = lookupGroupArray(
      this.data.packages,
      path,
      { enabled: false, shallow: false, exclude: [] },
    );
    return {
      enabled,
      shallow,
  ...
  getOutputPath() {
    ...
    if (this.data.output.filename !== null) {
      filename = this.data.output.filename;
    ...
    return Path.join(this.data.output.directory, filename);
  }
  getMetaData() {
    return {
      ...
      language: {
        ...
        engine: `${this.data.engine.name}@${this.data.engine.version}`,
      },
