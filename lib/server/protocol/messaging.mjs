...
const locateMessage = (message) => `failed to parse json message >> ${message}`;
...
export const attach = (server, dispatching) => {
  const respondAsync = ({ head, body }) =>
    dispatching.dispatchAsync(body).then((either) =>
      either.mapBoth((body) => ({
        head,
        body,
      })),
  ...
  server.on('connection', (socket) => {
    ...
    const sendEither = (either) => {
      either.either(sendLeft, sendRight);
    };
    NetSocketMessaging.patch(socket);
    socket.on('message', (message) => {
      logger.debug('receive: %j', message);
      toEither(JSON.parse, message)
        .mapLeft(locateMessage)
        .mapLeft(wrapNullHead)
        .bind(verify)
        .bindAsync(respondAsync)
        .then(sendEither);
    });
  });
};
