...
export default (path) => {
  ...
  return {
    repository: run(`git config --get remote.origin.url`, this.path),
    branch: run(`git rev-parse --abbrev-ref HEAD`, this.path),
    commit: run(`git rev-parse HEAD`, this.path),
    status: parseStatus(run(`git status --porcelain`, this.path)),
    tag: run(`git describe --abbrev=0 --tags`, this.path),
    annotated_tag: run(`git describe --abbrev=0`, this.path),
    commits_since_tag: parseDescription(run(`git describe --long --tag`, this.path)),
    commits_since_annotated_tag: parseDescription(run(`git describe --long`, this.path))
  };
};
