...
import { getFreshKey } from './get-fresh-key.mjs';

const VERSION = '1.4';

const navigate = (childeren, name) => {
  for (let index = 0; index < childeren.length; index += 1) {
...
export default (class Appmap {
  ...
  start(configuration) {
    configuration = this.configuration.extendsWithData(configuration, null);
  ...
  save(recording, callback) {
    ...
    if (this.cache.has(path)) {
      let counter = 0;
      while (this.cache.has(`${path}-${String(counter)}`)) {
        counter += 1;
      }
    ...
    logger.debug('Appmap recording save %s', path);
    if (callback === null) {
      FileSystem.writeFileSync(path, content, 'utf8');
    } else {
      FileSystem.writeFile(path, content, 'utf8', callback);
    }
  }
  stop(recording, callback) {
    if (recording === null) {
      ...
      if (this.recordings.has(recording)) {
        ...
        const error = new Error('Missing recording');
        if (callback === null) {
          throw error;
        }
        callback(error);
