...
const save = (recording, versioning) => {
  ...
  for (const {path, entities} of recording.origins) {
    split(Path.relative(recording.configuration.getBasePath(), Path.dirname(path)))
...
export class Recording {
  ...
  terminate (versioning) {
    ...
    try {
      ...
      return new Left(`failed to write appmap to file ${path} >> ${error.message}`);
    }
  ...
  terminateAsync (versioning) {
    ...
    return new Promise((resolve, reject) => {
      FileSystem.writeFile(path, content, 'utf8', (error) => {
        if (error) {
          resolve(new Left(`failed to write appmap to file ${path} >> ${error.message}`));
        } else {
