...
export class Appmap {
  ...
  instrument({source, path, content}) {
    ...
    const {enabled, shallow, exclude} = this.configuration.getInstrumentation(path);
    if (!enabled) {
  ...
  start ({data, path}) {
    assert(this.session !== null, "non-initialized appmap %o", this);
    assert(!this.terminated, "terminated appmap %o", this);
    return this.configuration.extendWithData(data, path).mapRight((configuration) =>
  ...
  stopAsync (key) {
    assert(this.session !== null, "non-initialized appmap %o", this);
    assert(!this.terminated, "terminated appmap %o", this);
    return this.recordings.take(key).bindAsync((recording) => recording.terminateAsync(this.versioning));
  }
