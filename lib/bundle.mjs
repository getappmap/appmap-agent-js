...
export default (namespace, options) => {
  ...
  return [
    ...
    Path.join(currated.platform, 'process-id.js'),
    ...
    'get-identity.js',
    'get-now.js',
    'serialize-exception.js',
    'serialize-parameter.js',
    'serialize.js',
    'undefined.js',
  ]
    .map((relative) =>
      FileSystem.readFileSync(Path.join(basename, relative), 'utf8'),
    )
    .join('')
    .replace(/APPMAP_[A-Z_]*/g, (identifier) =>
      namespace.compileGlobal(identifier),
    );
};
