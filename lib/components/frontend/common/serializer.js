...
const getInstanceID = (increment, value) => {
  if (typeof value === "symbol") {
    const id = symbols[value];
    if (id !== undefined) {
      return id;
    }
    return symbols[value] = increment();
  }
...
const createSerializer = () => {
  ...
  const empty = new Symbol("empty");
  const symbols = {__proto__:null};
  const references = new global_WeakMap();
  return {
    empty,
    serializeError: (value) => {
      ...
      if (value instanceof global_Error) {
        return {
          ...
          object_id: getInstanceID(increment, value),
