...
export const computeCaption = (node, index) => {
  ...
  if (
    ...
    return createCaption("Property", name);
  }
  if (
    ...
    return createCaption("MethodDefinition", name);
  }
  if (
    ...
    return createCaption("AssignmentExpression", parent.left.name);
  }
  if (
    ...
    return createCaption("VariableDeclarator", parent.id.name);
  }
  return createCaption(node.type, null);
};
