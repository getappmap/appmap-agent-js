...
import { createCaption, computeCaption } from "./caption.mjs";
import { createFunctionEntity } from "./entity.mjs";
import {
  setSimpleVisitor,
  setVisitor,
  visit,
...
const wrapClosure = (node, context, entities) => [
  createFunctionEntity(node, entities),
];
...
setVisitor(
  ...
  (node, context) =>
    node.id === null
      ? computeCaption(node)
      : createCaption("FunctionExpression", node.id.name),
  splitClosure,
  wrapClosure,
  joinClosure,
);
