...
if (path !== null) {
  ...
  if (extension === "") {
    stdout.write(`[AppmapAgent] Symbolic link ${stringify(link)} refers to ${stringify(path)} which has no extension, this is problematic for the node's esm loader which is enabled by --experimental-loader. To solve this issue, the agent is going to create copy of that file with a '.cjs' extension and overwrite the link to refer to that file. We hoop this is okay with you...${"\n"}`, "utf8");
    ...
    try {
      ...
      stdout.write(`[AppmapAgent] Something went wrong wen resolving the missing file extension issue: ${message}${"\n"}`, "utf8");
    }
  }
}
