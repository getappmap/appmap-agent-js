...
export default (dependencies) => {
  ...
  const { getClassmapParameters, getClassmapLink } = Classmap(dependencies);
  ...
  const compileBeforeEventData = (data, classmap) => {
    ...
    if (type === "apply") {
      ...
      return {
        ...getClassmapLink(classmap, _function),
        receiver: compileParameterSerial(["this", _this]),
        parameters: zip(
          getClassmapParameters(classmap, _function),
          _arguments,
        ).map(compileParameterSerial),
      };
    }
    if (type === "query") {
  ...
  const compileParameterSerial = ([
    name,
    { type, constructor: _constructor, index, truncated, print },
  ]) => ({
    name,
