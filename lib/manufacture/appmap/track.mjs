    ...
    splitTrack: (trace) => {
      ...
      for (let {type, data} of trace) {
        if (type === "track") {
          ...
          if (type === "start") {
            assert(!tracks.has(index), "duplicate track index: %i", index);
            const {options} = data;
            tracks.set(index, {
              options,
              enabled: true,
            ...
            assert(tracks.has(index), "missing track index: %i", index);
      ...
      return from(tracks.values()).map(({options, events}) => {options, events});
    }
  };
}
