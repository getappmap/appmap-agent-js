...
export default (options) => {
  ...
  if (currated.channel === 'local') {
    ...
    global[namespace.getGlobalAppmapObject()] = new Appmap(
      new Git(currated['git-path']),
      new Settings(process.env),
    );
    VirtualMachine.runScriptInThisContext(
      bundle(namespace, {
        ecma: currated.ecma,
    ...
    return VirtualMachine.runScriptInThisContext(instrument(file, namespace));
  }
