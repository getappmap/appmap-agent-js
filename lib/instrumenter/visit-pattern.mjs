...
import {getVoidContext, IdentifierContext} from "./context.mjs";
...
export default const visitPattern = (node, location) => {
  ...
  return visitExpression(node, location, getVoidContext());
};
...
const visitors = {
  ...
  Property: (node, location) => {
    let pair1;
    if (node.computed) {
      pair1 = visitExpression(node.key, location, getVoidContext());
    } else {
  ...
  AssignmentPattern: (node, location) => {
    ...
    if (node.left.type === "Identifier") {
      context = new IdentifierContext(node.left);
    }
