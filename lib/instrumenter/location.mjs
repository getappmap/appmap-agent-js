...
const getFieldName = (node) {
  if (node.key.type === "Literal" && Reflect.getOwnPropertyDescriptor(node, "regex") === undefined) {
...
export const makeRootLocation = (node, file, namespace) => new Location(node, null, {file, namespace});
...
export default class Location {
  ...
  getName () {
    ...
    if (
      ...
      kind = `${this.parent.node.static ? "constructor" : "prototype"}-${mapping[this.parent.kind]}`
    } else if (
      this.parent !== null &&
      ...
      name = getKeyName(this.parent.node);
      kind = `object-${mapping[this.parent.node.method ? "method" : this.parent.node.kind]}`;
    } else if (
      this.parent !== null &&
