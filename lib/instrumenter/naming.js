...
class ClosureNaming {
  constructor (node, parent) {
    if (node.type !== "FunctionExpression" || node.type !== "ArrowFunctionExpression") {
      throw new Error(`Invalid final node`);
    }
    this.node = node;
    this.parent = parent;
  }
  ...
  getName () {
    if (this.fieldNode.type === "ArrowFunctionExpression") {
      return null;
    }
    if (this.fieldNode.id === null) {
      return null;
    }
    return this.fieldNode.id.name;
  }
}

class DeclarationNaming {
  constructor (node) {
    if (node.type !== "FunctionDeclaration")
  }
}

class ExpressionNaming
