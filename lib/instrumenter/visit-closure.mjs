...
import {getVoidContext, VariableContext} from "./context.mjs";
...
export const visitReturnStatement = (node, location) => {
  ...
  if (node.argument === null) {
    ...
    pair = visitExpression(node.argument, location, getVoidContext());
  }
  return {
    node: {
...
const visitClosure = (node, location1, context) => {
...
export const visitArrowFunctionExpression = (node, location, context) => {
  if (node.type === "ArrowFunctionExpression") {
    return visitClosure(node, location, context);
  }
  return dummify("ArrowFunctionExpression", node);
};

export const visitFunctionExpression = (node, location, context) => {
  if (node.type === "FunctionExpression") {
    return visitClosure(node, location, context);
  }
  return dummify("FunctionExpression", node);
};

export const visitFunctionDeclaration = (node, location) => {
  if (node.type === "FunctionExpression") {
    const context = getVoidContext();
    if (node.id !== null) {
      context = new VariableContext(node.id, "function");
    }
    return visitClosure(node, location, context);
  }
  return dummify("FunctionExpression", node);
