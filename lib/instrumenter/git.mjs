...
const format = (command, reason, detail, stderr) =>
  `Command failure: ${command} >> ${reason} ${detail}${stderr === '' ? '' : `${'\n'}${stderr}`}`;
...
const spawnSync = (command) => {
  ...
  if (Reflect.getOwnPropertyDescriptor(result, 'error') !== undefined) {
    Log.warn(format(command, 'failed with', result.error.message, result.stderr));
    return null;
  }
  if (result.signal !== null) {
    Log.warn(format(command, 'killed with', String(result.signal), result.stderr));
    return null;
  }
  return result;
};
...
const run = (command) => {
  ...
  if (result.status !== 0) {
    Log.warn(format(command, 'exit with status', String(result.status), result.stderr));
    return null;
  }
...
export const getStatus = () => run('git status --porcelain').split('\n').map(trim);
