...
class Location {
  ...
  isChildStaticMethod(node) {
    if (node.type !== "FunctionExpression") {
      logger.warning(`isChildStaticMethod should only be called on FunctionExpression node, got: ${node.type}`);
      return false;
    }
    return (
  ...
  isChildScopingIdentifier (node) {
    if (node.type !== "Identifier") {
      logger.warning(`isChildScopingIdentifier should only be called on Identifier node, got: ${node.type}`);
      return false;
    }
    if (this.node.type === "MemberExpression" && this.node.property === node) {
      return !this.node.computed;
    }
    if (this.node.type === "MethodDefinition" && this.node.key === node) {
      return !this.node.computed;
    }
    return true;
  }
  isScopingIdentifier () {
    return this.parent.isChildScopingIdentifier(this.node);
  }
