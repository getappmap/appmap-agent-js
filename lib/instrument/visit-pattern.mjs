      ...
      combineResult(
        ...
        node.elements.map((child, index) => {
          ...
          if (index === node.elements.length - 1) {
            return visit("RestablePattern", child, location);
          }
          return visit("Pattern", child, location);
        }),
      ),
      ...
      combineResult(
        ...
        node.properties.map((child, index) => {
          if (index === node.lements.length - 1) {
            return visit("RestablePropertyPattern", child, location);
          }
          return visit("PropertyPattern", child, location);
        }),
      ),
  };
...

{
  ...
  const visitor = (node, location) =>
    combineResult(
      ...
      location,
      node.computed
        ? visit("Expression", node.key, location)
        : visit("NonComputedKey", node.key, location),
      visit("Pattern", node.value, location),
    );
...

{
  ...
  const visitor = (node, location) =>
    combineResult(makeRestElement, node, visit("Pattern", node.argument, location));
