    ...
    buildExpressionStatement(
      buildCallExpression(buildIdentifier(namespace.getGlobal('ADD_EVENT')), [
        buildObjectExpression([
          ...
          buildRegularProperty(
            'defined_class',
            buildLiteral(location.getClassName()),
          ),
            ...
            buildArrayExpression(
              node.params.map((child, index) =>
                buildCallExpression(
                  ...
                  [
                    buildIdentifier(
                      namespace.getLocal('ARGUMENT') + String(index),
                    ),
                    buildLiteral(
                      location
                        .getFile()
  ...
  const makeHeadStatementArray = (node, location, namespace, childeren) =>
    buildVariableDeclaration(
      ...
      childeren.map((child, index) =>
        buildVariableDeclarator(
          child,
          buildIdentifier(namespace.getLocal('ARGUMENT') + String(index)),
        ),
      ),
    );
  ...
  const makeVisitor = (kind) => (node, location) =>
    encapsulateResult(
      combineResult(
        ...
        node.params.map((child, index) =>
          visit(
            index === node.params.length - 1 ? 'RestablePattern' : 'Pattern',
            child,
            location,
          ),
        ),
