...
setVisitor("ReturnStatement", (node, context) => [
...

{
  ...
  const makeEnterStatement = (node, context) =>
    buildExpressionStatement(
      buildCallExpression(buildIdentifier(context.namespace.getGlobal('ADD_EVENT')), [
        buildObjectExpression([
          ...
          buildRegularProperty(
            'defined_class',
            buildLiteral(context.location.getParentContainerName(context.file)),
          ),
          buildRegularProperty('method_id', buildLiteral(context.location.getName(context.file))),
          buildRegularProperty(
            'path',
            buildLiteral(context.location.getFile().getPath()),
          ),
          buildRegularProperty('lineno', buildLiteral(context.location.getStartLine())),
          buildRegularProperty(
            'receiver',
            buildCallExpression(
              buildIdentifier(context.namespace.getGlobal('SERIALIZE_PARAMETER')),
              [
    ...
    buildExpressionStatement(
      buildCallExpression(buildIdentifier(context.namespace.getGlobal('ADD_EVENT')), [
        buildObjectExpression([
          ...
          buildRegularProperty(
            'parent_id',
            buildIdentifier(context.namespace.getGlobal('EVENT_IDENTITY')),
          ),
          buildRegularProperty(
            'ellapsed',
          ...
          buildRegularProperty(
            'exceptions',
            buildCallExpression(
              buildIdentifier(context.namespace.getGlobal('SERIALIZE_EXCEPTION')),
              [buildIdentifier(context.namespace.getLocal('FAILURE'))],
            ),
          ),
        ]),
  ...
  const makeHeadStatement = (node, context, childeren) =>
    buildVariableDeclaration(
      'var',
      childeren.map((child, index) =>
        buildVariableDeclarator(
          child,
          buildIdentifier(`${context.namespace.getLocal('ARGUMENT')}_${String(index)}`),
        ),
      ),
    );
  ...
  const joinClosure = (node, context, child1, childeren, child2) => ({
    ...
    body: buildBlockStatement([
      ...
      buildTryStatement(
        buildBlockStatement([
          makeHeadStatement(node, context, childeren),
          ...makeBodyStatementArray(node, context, child2),
        ]),
        buildCatchClause(
          buildIdentifier(context.namespace.getLocal('ERROR')),
          buildBlockStatement([
            makeFailureStatement(node, context),
          ]),
        ),
