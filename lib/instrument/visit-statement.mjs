...
import {assignVisitorObject, visitSwitchCase, visitVariableDeclaration, visitBlockStatement, visitCatchClause, visitScopingIdentifier, visitNonScopingIdentifier, visitVariableDeclarator, visitStatement, visitPattern, visitExpression, visitImportSpecifier, visitExportSpecifier, visitStringLiteral} from  "./visit.mjs";
...
{
  const makeBlockStatement = (node, childeren) => ({
    type: "BlockStatement",
    body: childeren
  });
...
{
  const makeCatchClause = (node, child1, child2) => ({
    type: "CatchClause",
    param: child1,
    body: child2
  });
...

{
  ...
  assignVisitorObject("ImportSpecifier", {
    ...
    ImportDefaultSpecifier: (node, location) => combineResult(
...

{
  ...
  const makeTryStatement = (node, child1, child2, child3) => ({
    type: "TryStatement",
    block: child1,
    handler: child2,
    finalizer: child3
