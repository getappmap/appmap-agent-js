...

{
  ...
  assignVisitorObject("ModuleDeclarationStatement", {
...

{
  ...
  const visitors = {
    ...
    // ImportDeclaration cf above
    // ExportAllDeclaration cf above
    // ExportDefaultDeclaration cf above
    // ExportNamedDeclaration cf above
    // Compound //
    ...
    ForStatement: (node, location) =>
      combineResult(
        makeForStatement,
        node.init === null ? getEmptyResult() : visit("DeclarableExpression", node.init.location),
  ...
  assignVisitorObject('Statement', visitors);
  
  assignVisitorObject('ModuleDeclarationStatement', visitors);
