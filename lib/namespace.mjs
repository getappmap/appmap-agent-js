...
const globals = {
  ...
  PROCESS_ID: null,
  SERIALIZE: null,
  SERIALIZE_ARGUMENT: null,
  SERIALIZE_ARGUMENT_ARRAY: null,
  SEND: null,
  GET_IDENTITY: null,
};
...
export default (class Namespace {
  ...
  getGlobal(name) {
    if (!(name in globals)) {
      logger.error(`Unrecognized global appmap name, got: ${name}`);
    }
    return `${this.prefix}_GLOBAL_${name}`;
  }
  getLocal(name) {
    if (!(name in locals)) {
      logger.error(`Unrecognized local appmap name, got: ${name}`);
    }
