import { assert, checkTypeError } from "../../../util/index.mjs";
...
const normalizeDatabaseArguments = (args) => {
  checkTypeError(args.length > 0, "missing sql query string");
  const sql = args[0];
  checkTypeError(
    typeof sql === "string",
...
exports.hookSQLite3 = (options, makeCouple) => {
  const recordSQLCall = (sql, parameters, callback) => {
    ...
    return function (...args) {
      assert(
        args.length === 0 ||
          args[0] === null ||
          args[0] === global_undefined ||
          (typeof args[0] === "object" &&
            global_Reflect_getOwnPropertyDescriptor(args[0], "message") !==
              undefined &&
      ...
      couple.recordReturn({
        sql_result: {
          error: args.length > 0 && args[0] ? args[0].message : null,
        },
      });
      return global_Reflect_apply(callback, this, args);
    };
  };
  const { Database } = requireFromHome("sqlite3");
  function Statement(database, sql, parameters, callback) {
    this._statement = global_Reflect_apply(save.prepare, database, [
      sql,
      callback,
    ]);
    this._sql = sql;
  ...
  global_Object_assign(Statement.prototype, {
    run: null,
    all: null,
    get: null,
  ...
  {
    const keys = global_Reflect_ownKeys(Statement.prototype);
    for (let index = 0; index < keys.length; index += 1) {
      const key = keys[index];
      if (Statement.prototype[key] === null) {
        Statement.prototype[key] = function (...args) {
          let { parameters, callback } = normalizeStatementArguments(args);
          parameters = combine(this._parameters, parameters);
          this._statement[key](
            parameters,
