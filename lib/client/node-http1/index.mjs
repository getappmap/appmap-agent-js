...
export default (dependencies) => {
  ...
  const rejectClientTermination = ({ interrupt, options, running }, error) => {
  ...
  return {
    ...
    executeClientAsync: async ({ interrupt, options }, { host, port }) => {
      setBox(options, {
        ...getBox(options),
    ...
    sendClient: (client, data) => {
      ...
      if (getBox(running)) {
        ...
        const request = connect(getBox(options));
