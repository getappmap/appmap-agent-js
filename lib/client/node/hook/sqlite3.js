...
const { assert, check } = require("../check.js");
...
const normalizeDatabaseArguments = (args) => {
  check(args.length > 0, "missing sql query string");
  const sql = args[0];
  check(
    typeof sql === "string",
...
const normalizeStatementArguments = (args) => {
  ...
  if (global_Array_isArray(parameters)) {
    for (let index = 0; index < parameters.length; index += 1) {
      check(isJSONPrimitive(parameters[index]), "invalid parameter type");
    }
  } else {
    const keys = global_Reflect_ownKeys(parameters);
    for (let index = 0; index < keys.length; index += 1) {
      check(isJSONPrimitive(parameters[keys[index]]), "invalid parameter type");
    }
  }
  return {
    parameters,
...
exports.hookSQLite3 = (recordCall) => {
  const recordSQLCall = (sql, parameters, callback) => {
    let recordReturn = recordCall("sql_query", {
