...
exports.hookHTTP = (options, makeCouple) => {
  let save = {
    client: [
  ...
  };
  {
    const hook = (request) => {
      ...
      request.on("finish", () => {
        const {protocol, host, pathname, query} = Url.parse(`${request.protocol}//${
        ...
        couple.recordCall({
          ...
          message: global_Object_assign({}, query)
        });
      });
    ...
    const hook = (server) => {
      server.on("request", (request, response) => {
        ...
        response.on("finish", () => {
          ...
          const {pathname, query} = Url.parse(`http://host${request.url}`, true);
          couple.recordCall({
            http_server_request: {
              headers: request.headers,
              authorization: (
                global_Reflect_getOwnPropertyDescriptor(request.headers, "authorization") !== global_undefined ?
                request.headers.authorization :
                null
              ),
              mime_type: (
                global_Reflect_getOwnPropertyDescriptor(request.headers, "content-type") !== global_undefined ?
