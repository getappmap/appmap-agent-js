...
const truncate = (string) => {
  if (string.length > VALUE_MAX_SIZE) {
    string = `${
      global_Reflect_apply(global_String_prototype_substring, string, [0, VALUE_MAX_SIZE / 2])
    }${
      VALUE_SEPARATOR
    }${
      global_Reflect_apply(global_String_prototype_substring, string, [string.length - (VALUE_MAX_SIZE / 2 - VALUE_SEPARATOR.length)])
    }`
  }
  return string;
}
