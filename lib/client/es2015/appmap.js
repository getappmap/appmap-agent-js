...
const resolveDisabledRecorder = (resolve, reject) =>
  resolve(getDisabledRecorder());
...
const make = ({ enabled, session, namespace }, channel, hook, run) => {
  ...
  runtime.record = (origin, event) =>
    channel.request({ action: "record", session, origin, event }, null);

  hook.start((source, path, content, pending) => {
    if (pending === null) {
...
exports.makeAppmap = (channel, hook, run, options) =>
