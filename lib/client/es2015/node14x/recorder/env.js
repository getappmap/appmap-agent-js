...
const toLowerCase = (string) => global_Reflect_apply(global_String_prototype_toLowerCase, string, empty);

const startsWith = (string, prefix) => global_Reflect_apply(global_String_prototype_startsWith, string, [prefix]);

const trim = (string) => global_Reflect_apply(global_String_prototype_trim, string, empty);

const splitComa = (string) => global_Reflect_apply(global_String_prototype_split, string, coma);

const parseBoolean = (string) => toLowerCase(string) === "true";

const parseArrayArgs = [trim];
const parseArray = (string) => global_Reflect_apply(
  ...
  for (let index = 0; index < keys.length; index += 1) {
    if (startsWith(keys[index], "APPMAP")) {
      if (!(keys[index] in mapping)) {
        throw new global_Error(`invalid appmap environment variable: ${keys[index]}`);
      }
