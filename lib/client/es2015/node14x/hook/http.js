...
exports.hookHTTP = (runtime) => {
  ...
  };
  {
    ...
    Http.get = new global_Proxy(save.get, traps);
    Http.request = new global_Proxy(save.request, traps);
    Http.ClientRequest = new global_Proxy(save.ClientRequest, traps);
  }
  {
    const hook = (server) => {
    ...
    Http.Server = new global_Proxy(save.Server, traps);
    Http.createServer = new global_Proxy(save.createServer, traps);
  }
  return () => {
    Assert.notEqual(save, null);
    Http.get = save.get;
    Http.request = save.request;
    Http.ClientRequest = save.ClientRequest;
    Http.createServer = save.createServer;
    Http.Server = save.Server;
    save = null;
  };
};
