...
export default (class AppMap {
  constructor(git, settings, file) {
    this.archived = false;
    this.settings = settings;
    this.json = {
      version: APPMAP_VERSION,
      metadata: {
        name: Path.basename(file.getPath(), ".js"),
        labels: ['TODO'],
  ...
  addEntity(entity) {
    if (this.archived) {
      ...
      this.json.classMap.push(entity);
    }
  }
  addEvent(event) {
    if (this.archived) {
  ...
  archive(termination) {
    ...
    if (this.archived) {
      ...
      FileSystem.writeFileSync(
        Path.join(
          this.settings.getOutputDir(),
          `${this.json.metadata.name}.appmap.json`,
        ),
