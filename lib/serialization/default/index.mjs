...
export default (dependencies) => {
  ...
  const serialize = (serialization, depth, value) => {
    ...
    if (type === "string") {
      const { maximum_string_length } = serialization;
      const { length } = value;
    ...
    let inspect = null;
    if (depth > 0) {
      const {maximum_property_count} = serialization;
      const keys = ownKeys(value).filter(isString);
      let truncated = false;
      if (keys.length > maximum_property_count) {
      ...
      inspect = {
        ...
        truncated,
        properties: fromEntries(keys.map((key) => [
          key,
  ...
  return {
    createSerialization: ({serialization:{"maximum-string-length":maximum_string_length, depth, "maximum-property-count":maximum_property_count}}) => ({
