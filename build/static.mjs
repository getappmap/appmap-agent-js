...
const visitComponentAsync = async (component, context) => {
  ...
  return {
    head: [
      ...
      ...instances.map(
        (instance) =>
          `import ${component}_${instance} from "./${relative(
            directory,
    ...
    body: [
      ...body,
      length === 1
        ? `dependencies.${component} = ${component}_${instances[0]}(dependencies);`
        : `dependencies.${component} = ${instances.reduce(
            (code, instance) =>
              `(blueprint.${component} === "${instance}" ? ${component}_${instance}(dependencies) : ${code})`,
